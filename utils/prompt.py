...
class promptGenerator:
    ...
    def generate_single_background_prompt(self, topics, goal, strategy, theme, n) -> str:
        ...
        ret =  PROMPT_TO_BACKGROUND[0] + topics + PROMPT_TO_BACKGROUND[1] +             goal + PROMPT_TO_BACKGROUND[2] + strategy + PROMPT_TO_BACKGROUND[3] +             theme + PROMPT_TO_BACKGROUND[4] + f"{n}" + PROMPT_TO_BACKGROUND[5]
        return ret
...
''' if failed_list != [] else ""
        ret = PROMPT_TO_QUESTION[0] + background + PROMPT_TO_QUESTION[1] +             preference + PROMPT_TO_QUESTION[2] + skip + PROMPT_TO_QUESTION[3]
        return ret

    def generate_dialogue_generation_prompt(self, scenario, question) -> str:
        if not self.setup:
            raise ValueError("Please set up the prompt generator with set_test() before generating prompts.")
        ret = DIALOGUE_GENERATION_PROMPT[0] + scenario + DIALOGUE_GENERATION_PROMPT[1] +             question + DIALOGUE_GENERATION_PROMPT[2]
        return ret
...
class promptChat:
    ...
    def generate_assistant_init_prompt(self, topics, goal, strategy, background) -> str:
        ret = ASSISTANT_INIT_PROMPT[0] + topics + ASSISTANT_INIT_PROMPT[1] +             goal + ASSISTANT_INIT_PROMPT[2] + strategy + ASSISTANT_INIT_PROMPT[3] + background + ASSISTANT_INIT_PROMPT[4]
        return ret
