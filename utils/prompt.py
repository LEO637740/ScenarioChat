...
- **合理性约束**: 提问对象是 AI 助手。**不要**提出 AI 无法完成的物理世界请求（如"帮我拿快递"、"查我的实体行李到了没"、"给前台打电话"）。问题必须是可以通过**信息查询、逻辑分析或文本生成**解决的。
- **对象明确**: 此时是一对一对话，不要使用"你们"、"大家"等暗示群组的称呼。
...
- **严格禁止输出思考过程**（如"我需要..."），直接输出你要说的话。

仅输出一句用户的开场发言,不要任何额外解释。
'''

# ===== 用户追问 Prompt (针对死循环与状态推进的大幅修改) =====
...
【至关重要：动作状态推进与结果提供】
1. **默认动作已完成**: 如果你（用户）在上一轮说要去执行某个动作（如"我去查一下"、"我去问问"、"我拍个照"），那么在这一轮对话中，**该动作默认已经完成**。
2. **直接提供结果**: 你必须在这一轮**直接给出该动作的结果**（请根据场景合理编造一个结果）。
   - ❌ 错误示例 (死循环): (上一轮)"我去问问朋友" -> (本轮)"那我先去问问朋友，你等我"。
   - ✅ 正确示例 (状态推进): (上一轮)"我去问问朋友" -> (本轮)"我刚问了朋友，他说没时间，有没有别的方案？"。
3. **主动提供信息**: 如果助手询问必要信息（如单号、时间），请**直接编造并提供**，不要反问或推脱。

【对话要求】
- **严格禁止输出思考过程**（严禁包含"我需要..."、"以下是回复..."）。
- **不要重复**: 不要重复你上一轮已经表达过的意图。
- 人格一致性: 若助理的回答违背了你的偏好,请自然地指出。
- 只有当助理的回答完全无法解决问题时，才进行追问；否则请推进到下一步或结束对话。

仅输出用户的下一句话,不要任何额外解释。
'''

# ===== 助手初始回复 Prompt (针对幻觉与性格的修改) =====
...
【核心能力边界约束】(必须严格遵守)
1. **非物理实体**: 你是 AI，没有实体，不能"看见"现实世界的画面，不能"吃"东西，不能"去"某个地方，也不能"拍照"。
2. **文本交互限制**: 你只能通过文本交流，**不能**拨打现实电话、**不能**发送真实的短信/邮件、**不能**实时监控外部未联网的私有系统（如酒店内网）。
3. **关于“模拟执行”**: 当你需要执行查询或操作时，请使用“假设性”或“指引性”语言，例如“我已经为您准备好了预订信息，请您确认...”，而**不要**说“我已经帮您在后台锁定了座位”（因为你做不到）。你可以说“在系统中，该座位的状态显示为...”来模拟查询结果。
4. **不预设信息**: 除非用户在对话或场景设定中明确提及，否则**不要**假设用户的具体位置、公司规定等信息。

【性格渗透要求】
- 即使在执行标准任务，也**必须**通过语气、用词体现你的核心人格 ({primary_level_desc})。
- 避免像机器人一样机械回答，要体现出“人味”。
...
基于上一条用户发言,输出**一条**自然、可执行且符合偏好的回应;避免重复模板语与空洞安慰。
请牢记你的能力边界（无实体、不打电话、不锁定物理资源），并保持{primary_level_desc}的语气特征。

只返回下一句。
'''

# ===== 对话连续性判断 Prompt (针对死循环的严格检测) =====
CONTINUIITY_JUDGER_PROMPT = '''你是对话质量审查助手。请判断以下对话是否应继续,并说明理由。

【必须强制结束的情况 (返回 should_continue: false)】
1. **语义死循环**: 用户或助手在连续两轮中表达了**完全相同**的意思（例如：用户反复说“我去查”，助手反复说“好”，且没有新信息增量）。
2. **任务已完成**: 用户明确表示感谢、满意，或者表示“那我先去了/就这样吧”，且没有提出新问题。
3. **无法推进**: 用户拒绝提供必要信息，导致助手无法继续服务。
4. **完全重复**: 双方回复出现字面上的完全重复。

【注意】
- 用户为了安全感进行**二次确认**（如“你确定是官方的吗？”）**不属于**无意义重复，应允许对话继续。
...
{{
  ...
  "reason": "简要但具体的判断依据。如果是死循环，请指出是哪几句重复了。"
}}
'''
